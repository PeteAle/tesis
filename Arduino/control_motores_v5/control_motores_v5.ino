...
#define comandoTicks 'T'          // Comando usado específicamente para el proyecto del brazo asistencial.
...
void ejecutarComandos(){
  ...
  if (primerMarcador >= 'a' && primerMarcador <= 'z'){
    if (eepromVal == primerMarcador){           // Compara el address recibido por serial con el address guardado en el eeprom.
      if (recibido[0] == comandoStep){          // Verifica si el comando es el de pasos.
        if (recibido[1] == '-'){                // Verifica si el símbolo que se envió para pasos es negativo.
          digitalWrite(dirPin, LOW);            // Establece la dirección de giro como clockwise.
          convertirPasos();
        ...
        else if (recibido[1] == '+'){           // Verifica si el símbolo que se envió para pasos es positivo.
          digitalWrite(dirPin, HIGH);           // // Establece la dirección de giro como counter-clockwise.
          convertirPasos();
          contadorPasos = pasosTotales;
          pasosTotales = 0;
          condicionStop(contadorPasos);
      ...
      else if (recibido[0] == comandoTicks){    // Verifica si el comando es el de ticks, para el mando manual.
        
      }
      else if (recibido[0] == comandoGetValue){ // Verifica si el comando es el de obtener el número de pasos anteriormente ejecutado por el motor.
        Serial.write(primerMarcador);
      ...
      else if (recibido[0] == comandoMicroStepping){  // Verifica si el comando recibido es el de establecer in nivel de microstepping.
        setMicroStepping();                           // Establece el microstepping. Los pines tienen que ir conectados al driver de la manera establecida.
      }
      else if (recibido[0] == comandoSetZero){  // Verifica si el comando recibido es el de establecer un cero relativo.
        setZero();
      }
